 ...
 * 
 */
class Bluehorse_Storelocator_Model_Resource_Storelocator extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    {
        $this->_init('bluehorse_storelocator/storelocator', 'storelocator_id');
    }
     ...
     * @return Bluehorse_Storelocator_Model_Resource_Storelocator
     */
    protected function _beforeDelete(Mage_Core_Model_Abstract $object)
    {
        ...
        $this->_getWriteAdapter()->delete($this->getTable('bluehorse_storelocator/storelocator_store'), $condition);
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        ...
        $table  = $this->getTable('bluehorse_storelocator/storelocator_store');
    ...
    public function lookupStoreIds($storelocatorId)
    {
        ...
        $select  = $adapter->select()
            ->from($this->getTable('bluehorse_storelocator/storelocator_store'), 'store_id')
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                array('bluehorse_storelocator_store' => $this->getTable('bluehorse_storelocator/storelocator_store')),
                $this->getMainTable() . '.storelocator_id = bluehorse_storelocator_store.storelocator_id',
                array())
                ->where('status = ?', 1)
                ->where('bluehorse_storelocator_store.store_id IN (?)', $storeIds)
                ->order('bluehorse_storelocator_store.store_id DESC')
                ->limit(1);
        }
        return $select;
    }
    ...
    protected function _getImportRow($row, $rowNumber = 0)
    {
        // validate row
        if (count($row) < 18) {
            $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid stores format in the Row #%s', $rowNumber);
            return false;
        }
        ...
        // validate store name
        if(empty($row[0])){
            $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid Store Name "%s" in the Row #%s.', $row[0], $rowNumber);
            return false;
        }
        // validate country
        $countryId = '';
        ...
        // validate state
        if(empty($row[2])){
            $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid State "%s" in the Row #%s.', $row[2], $rowNumber);
            return false;
        }
        
        // validate city
        if(empty($row[3])){
            $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid City "%s" in the Row #%s.', $row[3], $rowNumber);
            return false;
        }
        
        // validate Zipcode
        if(empty($row[4])){
            $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid Zipcode "%s" in the Row #%s.', $row[4], $rowNumber);
            return false;
        }
        
        // validate stores
        $isValidStoreString = Mage::helper('bluehorse_storelocator/validation')->isValidStores($row[5]);
        
        if($isValidStoreString === false){
          $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid Stores "%s" in the Row #%s.', $row[5], $rowNumber);
          return false;
        }
        ...
        // validate Status
           ...
           $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid Status "%s" in the Row #%s.', $row[6], $rowNumber);
            return false;
        }
        ...
        //validate url
        if(!empty($row[10])){
           $isValidSiteUrl = Mage::helper('bluehorse_storelocator/validation')->isValidUrl($row[10]); 
            if($isValidSiteUrl === false){
                $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid URL "%s" in the Row #%s.', $row[10], $rowNumber);
                return false;
           }
        }
        ...
        //validate email
        if(!empty($row[11])){
           $isValidEmailAddress = Mage::helper('bluehorse_storelocator/validation')->isValidEmail($row[11]); 
            if($isValidEmailAddress === false){
                $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid Email "%s" in the Row #%s.', $row[11], $rowNumber);
                return false;
           }
        }
        ...
        //validate image
        if(!empty($row[12])){
           $isValidImageName = Mage::helper('bluehorse_storelocator/validation')->isValidImage($row[12]); 
            if($isValidImageName === false){
                $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid Image "%s" in the Row #%s.', $row[12], $rowNumber);
                return false;
           }
        }
        //validate radius
        if(!empty($row[14])){
           if(!is_numeric($row[14])){
                $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid Radius "%s" in the Row #%s.', $row[14], $rowNumber);
                return false;
            }
        }
        ...
        if(!is_numeric($row[15])){
             $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid Latitude "%s" in the Row #%s.', $row[15], $rowNumber);
             return false;
         }
         
       //validate Longitude
        if(!is_numeric($row[16])){
             $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid Longitude "%s" in the Row #%s.', $row[16], $rowNumber);
             return false;
         }
        ...
        if(!empty($row[17])){
            if(!is_numeric($row[17])){
                $this->_importErrors[] = Mage::helper('bluehorse_storelocator')->__('Invalid Zoom Level "%s" in the Row #%s.', $row[17], $rowNumber);
                return false;
            }
        }
    ...
    protected function _saveImportData(array $stores)
    {
        if (!empty($stores)) {
            ...
            foreach($stores as $store){
                $storelocatorId ='';
                $storelocatorModel = Mage::getModel('bluehorse_storelocator/storelocator');
