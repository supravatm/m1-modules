...
jQuery(document).ready(function(){	
	jQuery("[rel='accordian']").each(function(){		
		jQuery(this).find("[rel='handle']").bind("click", function(){			
			var accordianGrid = jQuery(this).next();			
			if(jQuery(accordianGrid).css("display") == "none")
			{
				//jQuery("[class='panel-detail']").slideUp();
				jQuery(".panel-detail").slideUp();
				jQuery("[rel='handle']").removeClass("panel-title-active");
				jQuery("[rel='handle']").addClass("panel-title");
				jQuery(accordianGrid).slideDown();
				jQuery(this).attr("class","panel-title-active");
			}
			else
			{
				jQuery(accordianGrid).slideUp();
				jQuery(this).attr("class","panel-title");
			}
		});
	});
	
	jQuery("[rel='accordian-foot']").each(function(){		
		jQuery(this).find("[rel='handle']").bind("click", function(){			
			var accordianGrid = jQuery(this).next();			
			if(jQuery(accordianGrid).css("display") == "none")
			{
				//jQuery("[class='panel-detail']").slideUp();
				jQuery(".panel-detail").slideUp();
				jQuery("[rel='handle']").removeClass("title-active");
				jQuery("[rel='handle']").addClass("panel-title");
				jQuery(accordianGrid).slideDown();
			...
			{
				jQuery(accordianGrid).slideUp();
				jQuery(this).addClass("panel-title");
				//jQuery(this).attr("class","panel-title");
			}
		});
	});
